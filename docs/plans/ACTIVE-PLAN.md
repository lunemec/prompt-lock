...
## Fresh review notes
- A latest 2026-03-15 backlog pass closed `QA-009`, `QA-010`, `UX-009`, `QA-011`, `ARCH-005`, and `API-007`: readiness-check now fails closed on empty or schema-light scoreboards, live redteam requires network-egress-specific deny evidence, watch/watch-list show approved intent, the script-based redteam gate now covers the wired approved-lease no-inspectable-destination deny path, execution use-cases take ambient env only from the broker boundary, and request/contract docs no longer advertise client-supplied `env_path_canonical`.
- A latest 2026-03-15 strict re-review follow-up opened `SEC-035`, `QA-008`, `UX-008`, and `DOC-006`; all are now closed. Broker-exec now resolves commands before any secret backend or approved env-path read, `promptlock-redteam-live` now uses a real approved request/lease for the live SEC-031 execute-time egress deny check, Unix-socket CLI auth/watch/exec flows now ignore ambient `PROMPTLOCK_BROKER_URL` when socket transport is selected, and README/CHANGELOG transport wording now matches the active local-only hardened dual-socket product surface.
- A fresh 2026-03-15 strict review opened `SEC-031`, `SEC-032`, `SEC-033`, `SEC-034`, `ARCH-004`, `UX-007`, `QA-006`, and `QA-007`; all of those remediation items are now closed, and `docs/plans/BACKLOG.md` currently has no open follow-up items from that review.
...
## Recently completed
- Closed `QA-009` by making `promptlock-readiness-check` reject `{}`, empty `tasks`, and schema-light task lists with no release-gating entries under the legacy `--require-p0` gate, with direct regression coverage.
- Closed `QA-010` by making `promptlock-redteam-live` require network-egress-specific deny evidence from the approved-lease SEC-031 probe and adding regressions that reject generic `403`, execution-policy, and command-resolution failures as false positives.
- Closed `UX-009` by surfacing approved request `intent` in `promptlock watch` and `promptlock watch list`, and by extending queue-context tests so operators see intent in approval context.
- Closed `QA-011` by extending the script-based `security-redteam` gate with the wired approved-lease no-inspectable-destination execute deny path instead of relying only on the policy helper.
- Closed `ARCH-005` by removing app-layer `os.Environ()` fallback from execution use-cases, injecting ambient env from the broker boundary, and tightening architecture conformance around app-layer OS coupling and handler-side control-plane construction.
- Closed `API-007` by removing `env_path_canonical` from the advertised request schema, documenting the ignored legacy compatibility behavior, and tracking the remaining `--require-p0` naming cleanup as `COMPAT-001` in `BACKLOG.md`.
- Closed `SEC-035` by reordering broker-exec app-layer execution so command resolution succeeds before any secret backend or approved env-path read, with regression coverage that proves command-resolution failure now blocks secret reads and suppresses `secret_access_started` / `secret_access`.
- Closed `QA-008` by making `cmd/promptlock-redteam-live` create and approve a real lease before the live SEC-031 deny probe and by adding direct harness coverage plus an approved-lease execute deny regression in `cmd/promptlockd`.
- Closed `UX-008` by making Unix-socket CLI auth/watch/exec request construction ignore ambient `PROMPTLOCK_BROKER_URL`, preferring explicit/role/default socket transport when selected and adding malformed-URL regressions for explicit socket, role-socket watch, and role-socket auth flows.
- Closed `DOC-006` by aligning README dev/demo guidance and `[Unreleased]` transport notes with the current local-only hardened dual-socket story and removing stale TLS/mTLS transport claims from the active changelog surface.
- Closed `QA-006` by adding direct release/security-gate tests for malformed readiness files and blocking classification, strengthening `promptlock-redteam-live` coverage around report-write failures, readiness/startup negative paths, helper failure handling, log tailing, and auth-stage aborts, and preserving the bounded MCP/CLI timeout coverage added for `UX-007`.
- Closed `QA-007` by making `make lint` syntax-check every shipped shell workflow under `scripts/` and by extending the script-based SEC-031 red-team gate to cover the direct-network-client-without-inspectable-destination deny case.
- Closed `SEC-031` by persisting approved intent on requests/leases, binding execute-time egress checks to that approved intent, failing closed for direct network clients without inspectable destinations, and updating contract/ops guidance to match the enforced boundary.
- Closed `SEC-032` by classifying durability-gate audit reasons into stable sanitized codes so raw upstream backend error text no longer lands in host audit metadata.
- Closed `SEC-033` by making `promptlock-validate-security` fail closed on unreadable or otherwise unscannable files, while documenting and testing the explicit skip classes.
- Closed `SEC-034` by replacing narrow literal-only output masking with token-aware best-effort redaction for common bearer/env-style secret shapes and keeping hardened docs centered on `output_security_mode=none` for stronger containment.
- Closed `ARCH-004` by moving broker execute and host-docker orchestration into app-layer use-cases, wiring env-path secret resolution in the composition root, and adding startup/unit regression coverage around the new boundaries.
- Closed `UX-007` by adding bounded `10s` broker client deadlines for CLI and MCP TCP/Unix-socket calls, along with stalled-peer regression coverage and clearer timeout errors.
