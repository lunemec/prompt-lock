# 0020 - Production hardening: release gates and persistence safety
...
- Tagged release workflow did not enforce all checklist gates (`fuzz`, release-grade smoke) before packaging.
...
## Decision
1. Add `make release-readiness-gate` (`validate-final`, readiness gate, fuzz, supported hardened dual-socket smoke via `make real-e2e-smoke`) and run it in tagged release CI before fsync attestation/package steps.
...
- Improves release-time assurance by requiring fuzz and a smoke gate that matches the supported hardened transport story.
