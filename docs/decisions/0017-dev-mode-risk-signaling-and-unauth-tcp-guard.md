...
## Decision
1. Add fail-fast startup guard for `auth.enable_auth=false` on non-local TCP when neither unix socket nor TLS is configured.
...
- Slightly stricter behavior for no-auth deployments that relied on non-local TCP without secure transport.
