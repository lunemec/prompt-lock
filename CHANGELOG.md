...
### Fixed
- `docker-compose.e2e.yml` now pins the broker service to the Dockerfile `runtime` stage and the smoke runner to `agent-lab`, so `make e2e-compose` no longer depends on the Dockerfile's last stage and the broker container no longer exits immediately with `/bin/sh`.
