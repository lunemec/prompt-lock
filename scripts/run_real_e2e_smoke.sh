...
find_pending_request_id() {
  ...
  for _ in $(seq 1 "$attempts"); do
    out="$("$PROMPTLOCK_BIN" watch list --operator-token "$OP_TOKEN" 2>/dev/null || true)"
...
PTY_RUNNER="$TMPDIR/promptlock-pty-runner"
go build -o "$PTY_RUNNER" ./cmd/promptlock-pty-runner
PROMPTLOCK_BIN="$TMPDIR/promptlock"
go build -o "$PROMPTLOCK_BIN" ./cmd/promptlock

run_pty_command() {
  local transcript_file="$1"
  local input_text="$2"
  shift 2
  "$PTY_RUNNER" --output "$transcript_file" --timeout 30s --inputs "$input_text" "$@"
}
...
for _ in $(seq 1 50); do
  READY_OUT="$("$PROMPTLOCK_BIN" watch list --operator-token "$OP_TOKEN" 2>&1)" && {
    READY=true
    break
  } || READY_OUT="$READY_OUT"
  sleep 0.2
...
boot=$("$PROMPTLOCK_BIN" auth bootstrap --operator-token "$OP_TOKEN" --agent "$AGENT_ID" --container "$CONTAINER_ID" | jq -r '.bootstrap_token')
grant=$("$PROMPTLOCK_BIN" auth pair --token "$boot" --container "$CONTAINER_ID" | jq -r '.grant_id')
session=$("$PROMPTLOCK_BIN" auth mint --grant "$grant" | jq -r '.session_token')

export PROMPTLOCK_SESSION_TOKEN="$session"

# interactive TTY approval path
("$PROMPTLOCK_BIN" exec --agent "$AGENT_ID" --task "smoke-watch-tty" --intent run_tests --reason "smoke tty watch" --ttl 5 --wait-approve 20s --poll-interval 1s --broker-exec -- go version >"$TMPDIR/watch-tty.out" 2>"$TMPDIR/watch-tty.err") &
TTY_PID=$!
...
if [[ -n "$TTY_REQ_ID" ]]; then
  ...
  if run_pty_command "$TTY_WATCH_OUT" $'y\nq\n' "$PROMPTLOCK_BIN" watch --operator-token "$OP_TOKEN"; then
...
("$PROMPTLOCK_BIN" exec --agent "$AGENT_ID" --task "smoke-allow" --intent run_tests --reason "smoke allow" --env-path "secrets/github.env" --ttl 5 --wait-approve 20s --poll-interval 1s --broker-exec -- go version >"$TMPDIR/allow.out" 2>"$TMPDIR/allow.err") &
ALLOW_PID=$!
...
# deny path
("$PROMPTLOCK_BIN" exec --agent "$AGENT_ID" --task "smoke-deny" --intent run_tests --reason "smoke deny" --ttl 5 --wait-approve 10s --poll-interval 1s --broker-exec -- echo deny_should_fail >"$TMPDIR/deny.out" 2>"$TMPDIR/deny.err") &
DENY_PID=$!
...
("$PROMPTLOCK_BIN" auth docker-run \
